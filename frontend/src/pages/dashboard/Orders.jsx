...
import { Loader2, Search, Plus, ExternalLink, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-6" data-testid="orders-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link to="/dashboard/orders/add">
          <Button className="bg-[#7c3aed] hover:bg-[#8b5cf6] text-white rounded-[8px]" data-testid="add-order-btn">
        ...
        <CardContent className="p-4">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={statusFilter} onValueChange={(val) => { setStatusFilter(val === 'all' ? '' : val); setPage(1); }}>
              <SelectTrigger className="w-full sm:w-[180px] h-10 rounded-[8px] border-[#e5e7eb]" data-testid="status-filter">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#e5e7eb] bg-[#f9fafb]">
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">ID</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">Created</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">Link</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">Service</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">Qty</th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-[#6b7280] uppercase tracking-wider">Price</th>
                    ...
                    <tr key={order.id} className="border-b border-[#e5e7eb] last:border-0 hover:bg-[#f9fafb]">
                      ...
                      <td className="py-4 px-4">
                        <a 
                          ...
                          className="flex items-center gap-1 text-sm text-[#7c3aed] hover:text-[#8b5cf6] max-w-[200px] truncate"
                        >
